<template>
  <div class="charts-ls-fssl">
    <div id="cc4" class="canvas"></div>

    <div id="cc4_legend"></div>
    <div
      id="ccbt4_legend"
      @click="cbt_legend_click('ccbt4_legend', 'cc4_legend')"
      @mouseover="cbt_legend_over('ccbt4_legend')"
      @mouseleave="cbt_legend_leave('ccbt4_legend')"
    >
      更多
    </div>
  </div>
...
watch(getIsCollapse, (newQuestion, oldQuestion) => {
  setTimeout(() => {
    draw_LS_EVFSSL(items.value, items_es_id_ls.value);
  }, 300);
});

// 渲染logstash Events发送速率 EVFSSL
function draw_LS_EVFSSL(data, es_id_ls) {
  ...
  let chartDom = document.getElementById("cc4") as HTMLDivElement;
  let myChart_cc4 = echarts.getInstanceByDom(chartDom) as any;
  if (myChart_cc4) {
    echarts.dispose(myChart_cc4);
    myChart_cc4 = null;
  }
  chartDom.removeAttribute("_echarts_instance_");
  myChart_cc4 = echarts.init(chartDom);
  // console.log(arr);
  // 指定图表的配置项和数据
  var option = {
    tooltip: {
    ...
    formatter: function (params) {
      ...
      // 获取目标容器
      let htmlObj = document.getElementById("cc4_legend") as HTMLDivElement;
  ...
  // 使用刚指定的配置项和数据显示图表。
  option && myChart_cc4.setOption(option);
  myChart_cc4.group = "group_node_2";
  window.addEventListener("resize", () => myChart_cc4.resize());
