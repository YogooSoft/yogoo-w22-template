<template>
  <div class="charts-es-xrsl">
    <div id="c2" class="canvas"></div>
    <div id="c2_legend"></div>
    <div
      id="cbt2_legend"
      @click="cbt_legend_click('cbt2_legend', 'c2_legend')"
      @mouseover="cbt_legend_over('cbt2_legend')"
      @mouseleave="cbt_legend_leave('cbt2_legend')"
    >
      更多
    </div>
  </div>
...
watch(getIsCollapse, (newQuestion, oldQuestion) => {
  setTimeout(() => {
    draw_XRSL(items.value);
  }, 300);
});

// 渲染写入速率图
function draw_XRSL(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    let d = [];
    d = data[i].data.indexing_rates_pri;
    let t = [];
    ...
    var series_json = {
      name: data[i].name + "(indexing_rates_pri)",
      type: "line",
      stack: "z1",
      symbolSize: 3, //拐点大小
      data: arr,
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    let d = [];
    d = data[i].data.indexing_rates_tot;
    let t = [];
    t = data[i].data.time_format;
    const arr: any[] = [];
    t.map((v, index) => {
    ...
    var series_json = {
      name: data[i].name + "(indexing_rates_tot)",
      type: "line",
      stack: "z1",
      symbolSize: 3, //拐点大小
      data: arr,
    ...
    // legend_list.push(data[i].name + "(indexing_rates_tot)");
    series_list.push(series_json);
  }

  let chartDom = document.getElementById("c2") as HTMLDivElement;
  let myChart = echarts.getInstanceByDom(chartDom) as any;
  if (myChart) {
  ...
  var option = {
    tooltip: {
      ...
      formatter: function (params) {
        ...
        // 获取目标容器
        let htmlObj = document.getElementById("c2_legend") as HTMLDivElement;
