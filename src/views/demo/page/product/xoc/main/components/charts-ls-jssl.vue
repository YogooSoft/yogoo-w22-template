<template>
  <div class="charts-ls-jssl">
    <div id="cc3" class="canvas"></div>

    <div id="cc3_legend"></div>
    <div
      id="ccbt3_legend"
      @click="cbt_legend_click('ccbt3_legend', 'cc3_legend')"
      @mouseover="cbt_legend_over('ccbt3_legend')"
      @mouseleave="cbt_legend_leave('ccbt3_legend')"
    >
      更多
    </div>
  </div>
...
watch(getIsCollapse, (newQuestion, oldQuestion) => {
  setTimeout(() => {
    draw_LS_EVJSSL(items.value, items_es_id_ls.value);
  }, 300);
});

// 渲染logstash Events接收速率 EVJSSL
function draw_LS_EVJSSL(data, es_id_ls) {
  const series_list: any[] = [];
  const legend_list: any[] = [];
  items.value = data;
  ...
  for (let i = 0; i < data.length; i++) {
    if (es_id_ls.includes(data[i].es_cluster)) {
      ...
      legend_list.push(data[i].es_cluster);

      series_list.push(series_json);
      // console.log(data,"=====data==draw_LS_EVJSSL==3=",series_list)
    } else {
    }
  }

  let chartDom = document.getElementById("cc3") as HTMLDivElement;
  let myChart_cc3 = echarts.getInstanceByDom(chartDom) as any;
  if (myChart_cc3) {
    echarts.dispose(myChart_cc3);
    myChart_cc3 = null;
  }
  chartDom.removeAttribute("_echarts_instance_");
  myChart_cc3 = echarts.init(chartDom);
  // console.log(arr);
  // 指定图表的配置项和数据
  var option = {
    tooltip: {
    ...
    formatter: function (params) {
      ...
      // 获取目标容器
      let htmlObj = document.getElementById("cc3_legend") as HTMLDivElement;
  ...
  // 使用刚指定的配置项和数据显示图表。
  option && myChart_cc3.setOption(option);
  myChart_cc3.group = "group_node_2";
  window.addEventListener("resize", () => myChart_cc3.resize());
