<template>
  <div class="view-home">
    <el-card :body-style="{ padding: '0px', height: '100%' }">
      <div class="card">
        <span class="card__header">本月公司与市场余额对比趋势图</span>
      </div>
...
import { GetBalanceGSEchartsData } from "@/api/fislData";
...
function chartsBtn() {
  ...
  GetBalanceGSEchartsData("userid").then((res: any) => {
    res = res.data.data;
    echartData["pop"]["xData"] = res.dates;
    echartData["pop"]["series1"] = res.balance_gs;
    echartData["pop"]["series2"] = res.balance_sc;
