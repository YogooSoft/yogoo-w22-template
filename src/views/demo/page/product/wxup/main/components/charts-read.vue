<template>
  <div class="view-home">
    <el-card :body-style="{ padding: '0px', height: '100%' }">
      <div class="card">
        <span class="echarts_title">公众号推文浏览量趋势图(次) </span>
        <!-- <sapn class="echarts_dw">(单位：次)</sapn> -->
      </div>
...
import { GetWxupReadEchartsData } from "@/api/wxupData";
...
function chartsBtn() {
  GetWxupReadEchartsData("userid").then((res: any) => {
    res = res.data.data;
    console.log("=====", res);
    echartData["pop"]["xData"] = res.dates;
    echartData["pop"]["series1"] = res.cs;
    echartData["pop"]["series2"] = res.rc;
