<template>
  <div class="view-home">
    <el-card :body-style="{ padding: '0px', height: '100%' }">
      <div class="card">
        <span class="echarts_title">资金账号绑定量(人)</span>
        <!-- <sapn class="echarts_dw">(单位：人)</sapn> -->
      </div>

      <div id="dc3" width="300" height="178" class="canvas"></div>
    </el-card>
  </div>
</template>
...
import { GetWxupAccountData } from "@/api/wxupData";
...
function chartsBtn() {
  GetWxupAccountData("userid").then((res: any) => {
    res = res.data.data;
    console.log("=====", res);
    echartData["pop"]["xData"] = res.dates;
    echartData["pop"]["series1"] = res.bd;
