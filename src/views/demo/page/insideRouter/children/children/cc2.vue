<template style="border: 1px solid red">
  <el-alert title="主页面>>>二级路由页面02>>>三级路由页面02" style="margin-bottom: 10px"  type="info" :closable="false" />
  <collapse-container
    class="cl-mb-10"
    title="主页面的查询条件[json]"
    :can-expan="false"
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    methods: {},
    mounted() {
      // console.log("===teamInfoBKData=cc2==", this.teamInfoBKData)
      // this.psnTableInfo = this.teamInfoBKData
      try {
        if (this.teamInfoBKData['c2']['cc2']) {
          this.psnTableInfo = this.teamInfoBKData['c2']['cc2']
        }
    ...
    watch: {
      psnTableInfo: {
        handler(newValue, oldValue) {
          // console.log("===getTeamInfo==cc2=", newValue)
