...
let editor;
var myBinding;
heights = `${unref(window.innerHeight - 81 - 153)}px`;
let editorContainer = ref<any>(null);

if (!monaco.languages.register["mySpecialLanguage"]) {
  monaco.languages.register["mySpecialLanguage"] = true;
...
if (!monaco.editor.defineTheme["myCoolTheme"]) {
  monaco.editor.defineTheme["myCoolTheme"] = true;
...
if (!monaco.languages.registerCompletionItemProvider["mySpecialLanguage"]) {
  monaco.languages.registerCompletionItemProvider["mySpecialLanguage"] = true;
...
var jsonModel = monaco.editor.createModel(
  getCode(),
  "mySpecialLanguage",
  monaco.Uri.parse("files://log/system.js")
);
