<template>
  <page-wrapper title="yuiTable组件" content="">
    <collapse-container
      ...
      :can-expan="true"
    >
      <el-alert title="高度：设置表格固定高度和最大高度；行文本位置：表头居中、行数据居中、表尾居中；表尾合计：使用接口合计数据；展开行数据：可配置默认展开；翻页：前端控制翻页，位置居右，此处配置为当只有一页时隐藏分页；"
                type="info" show-icon />
  ...
  <page-wrapper title="yuiTable组件03" content="">
    <collapse-container
      class="cl-mb-10"
      title="筛选、自定义列模板、自定义表头、行文本位置"
      :can-expan="true"
    >
      <el-alert title="筛选：建议此处使用状态、等级、性别等分类较少的简单筛选，更复杂的筛选功能在搜索区域实现；行文本位置：表头居左、行数据居中、表尾居右；"
                type="info" show-icon />

      <yui-table
        :rowData="rowData3.filter(data => !search || data.userId.toLowerCase().includes(search.toLowerCase()))"
  ...
  <page-wrapper title="yuiTable组件02" content="">
    <collapse-container
      class="cl-mb-10"
      title="带状态表格、树形结构、懒加载、多选、多级表头、固定列、翻页(后端控制)"
      :can-expan="true"
    >
      <el-alert title="固定列：可配置固定位置居左居右，带状态表格不对固定的列生效；翻页：后端控制翻页，位置居左，当只有一页时不隐藏分页；"
                type="info" show-icon />
